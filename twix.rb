...
module Twix  
  ...
  def self.setup
    config = YAML::load_file(File.dirname(__FILE__) + "/twitter_config.yaml")
  ...
  def self.get_pics(query,how_many=1)
    ...
    tweets = Twitter.search("#{URI.escape query} AND (instagr.am OR pic.twitter.com)", :include_entities=>true, :count=>how_many)
